...
def запустить_пайплайн(input_path):
    ...
    output_dir = project_root / "results" / "consensuses_fastas"
    ...
    for s_id in sorted(strain_groups.keys(), key=lambda x: int(x)):
        ...
        if f_match and r_match:
            fp, rp = f_match[0], r_match[0]  # Исправлен баг индексации
        elif len(files) == 2:
            ...
            if s1 and s2_raw:
                ...
                for char1, char2 in zip(aln_s1, aln_s2):
                    ...
                    if char1 == char2 and char1 != '-':
                        consensus.append(char1);
                        quals.append(max(v1, v2))
                    elif char1 == '-':
                        consensus.append(char2);
                        quals.append(v2)
                    elif char2 == '-':
                        consensus.append(char1);
                        quals.append(v1)
                    # --- НОВЫЙ БЛОК ОБРАБОТКИ 'N' ---
                ...
                SeqIO.write(rec, output_dir / f"Consensus_{s_id}.fasta", "fasta")
                print(f"✅ ID {s_id}: Собрано (Тримминг и N-коррекция применены)")
        except Exception as e:
            print(f"❌ Ошибка в ID {s_id}: {e}")

    # 3. ВИЗУАЛИЗАЦИЯ (Твой блок без изменений)
    if report_data:
        ...
        legend_elements = [
            ...
            Line2D([0], [0], color='#f1c40f', lw=8,
                   label=f'Посредственный (Q20-30): {counts.get("Посредственный", 0)}'),
            Line2D([0], [0], color='#e74c3c', lw=8, label=f'Непригодный (Q<20): {counts.get("Непригодный", 0)}')
        ]

        ax.legend(handles=legend_elements, title="ЛЕГЕНДА", bbox_to_anchor=(1.02, 1), loc='upper left', frameon=True,
                  fontsize=10)
        ...
        print("❌ Ошибка: Данные не найдены. Проверьте названия файлов.")
